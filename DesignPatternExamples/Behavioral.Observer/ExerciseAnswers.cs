    ...
    namespace Coding.Exercise.Tests
    {
        ...
        public class Tests
        {
            ...
            public void SingleRatTest()
            {
                ...
                var ratRef = new WeakReference(rat);
                rat = null; // memory leak

                GC.Collect();
                GC.WaitForPendingFinalizers();
                GC.Collect();
                Assert.That(ratRef.IsAlive, Is.True);

                var gameRef = new WeakReference(game);
                game = null;

                GC.Collect();
                GC.WaitForPendingFinalizers();
                GC.Collect();
                Assert.That(gameRef.IsAlive, Is.False);
            }
